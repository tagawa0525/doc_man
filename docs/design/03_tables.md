...
| `status`           | `VARCHAR(20)`  | NO   | `'draft'`           | 承認ステータス                                 |
...
承認の状態遷移は `status` で管理し、状態遷移ルールは `05_approval_flow.md` に従う。
...
| `id`             | `UUID`        | NO   | `gen_random_uuid()` | PK             |
| `document_id`    | `UUID`        | NO   | -                   | 対象文書       |
| `recipient_id`   | `UUID`        | NO   | -                   | 配布先社員     |
| `distributed_at` | `TIMESTAMPTZ` | NO   | `now()`             | 配布日時       |
| `distributed_by` | `UUID`        | NO   | -                   | 配布実行者     |
| `created_at`     | `TIMESTAMPTZ` | NO   | `now()`             | レコード作成日 |

**制約**:

- `FK document_id → documents(id)`
- `FK recipient_id → employees(id)`
- `FK distributed_by → employees(id)`

**インデックス**:

- `(document_id)` — 文書別配布履歴
- `(recipient_id)` — 社員別配布履歴

**設計理由**:
承認フローとは独立した配布機能。文書のステータスに関わらず配布可能（draft でも approved でも可）。
同一 API リクエスト内の配布先は同一 `distributed_at` タイムスタンプを共有し、バッチとしてグルーピングされる。
受領確認は追跡しない（メール送信のみ）。
