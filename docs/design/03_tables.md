...
| --------------- | -------------- | ---- | ------------------- | ------------------------------------------------------ |
| `id`            | `UUID`         | NO   | `gen_random_uuid()` | PK                                                     |
| `code`          | `VARCHAR(10)`  | NO   | -                   | 種別コード（採番用）。例: `CAD`, `FEM`, `MECH`, `ELEC` |
...
**設計理由**:
業種ではなく業務プロセスの分類。`code` を文書番号の種別コードとして使用するため不変にする。
...
- `(business_category_id)` -業務種別別文書一覧
- `(author_id)` -作成者別文書一覧
- `(confidentiality)` -機密グレードフィルタ

**設計理由**:
`frozen_dept_code` は業務種別の担当部署コードを登録時にコピーして凍結する。
部署再編後も文書番号と帰属部署を変更する必要がなく、過去の文書番号体系を維持できる。
`PUT /documents/:id` では `business_category_id`, `frozen_dept_code`, `doc_number` を変更不可とする。
