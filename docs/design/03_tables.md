...
**設計理由**:
段階承認を `route_revision + step_order` で表現する。差し戻し再提出時は旧ルートを削除せず、`route_revision` を増やして新ルートを追加する。現在のアクティブステップは最新 `route_revision` 上で特定する。承認フローの詳細は `05_approval_flow.md` を参照。
