...
pub async fn create(doc_id: Uuid, req: &CreateCirculationRequest) -> Result<Vec<CirculationResponse>, ApiError> {
...
pub async fn confirm(doc_id: Uuid) -> Result<CirculationResponse, ApiError> {
    client::post(&format!("/api/v1/documents/{doc_id}/circulations/confirm"), &serde_json::json!({})).await
}
